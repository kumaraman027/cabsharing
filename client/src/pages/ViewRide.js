...
export default function ViewRide() {
  ...
  const fetchRides = async () => {
    try {
      const res = await axios.get(`${API}/api/ride/all`);
  ...
  const fetchJoinedRequests = async () => {
    ...
    try {
      const res = await axios.get(`${API}/api/join/user/${user.email}`);
  ...
  const fetchJoinCounts = async () => {
    try {
      const res = await axios.get(`${API}/api/join/all`);
      const counts = {};
        ...
        <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {filteredRides.map((ride) => {
            ...
            return (
              <div
                key={rideId}
                className="ride-card bg-white rounded-lg p-4 shadow-lg"
              >
                <div className="ride-header font-semibold text-indigo-700">
                  {ride.from_location} → {ride.to_location}
                </div>

                <div className="ride-date text-gray-600 mb-2">
                  {ride.date?.slice(0, 10)} at {ride.time?.slice(0, 5)}
                </div>

                <div className="ride-details text-gray-700 space-y-1">
                  <div>
